(async function() {
    ...
    try {
        while (isRunning) {
            ...
            // CLEANUP 1: Human-Like Jitter
            const jitter = Math.floor(Math.random() * 300) + 500; 
            await new Promise(r => setTimeout(r, jitter)); 
        }

        // CLEANUP 4: Large Archive Handling
        if (allMessages.length > 10000) {
            document.getElementById('status-text').innerText = "Processing large file... please wait.";
        }
        ...
        if (allMessages.length > 0) {
            ...
            // CLEANUP 3: CSP Meta Tag added to <head>
            let htmlContent = `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="referrer" content="no-referrer"><title>#${channelName}</title>
            <style>
            ...
            allMessages.forEach(msg => {
                ...
                // CLEANUP 2: Robust data-content encoding
                const encodedContent = encodeURIComponent((msg.content || '').toLowerCase());

                if (msg.author.id === lastAuthorId) {
                    htmlContent += `<div class="chatlog__message-container" data-content="${encodedContent}">
                    ...
                    htmlContent += `<div class="chatlog__message-container" style="margin-top:12px;" data-content="${encodedContent}">
            ...
            // CLEANUP 2: Updated search script with decodeURIComponent
            htmlContent += `</div></div>
            ...
            <script>
                ...
                document.getElementById('searchInput').oninput = (e) => {
                    ...
                    document.querySelectorAll('.chatlog__message-container').forEach(m => {
                        const content = decodeURIComponent(m.getAttribute('data-content'));
                        m.classList.toggle('hidden', !content.includes(val));
                    });
                };
            <\/script></body></html>`;
